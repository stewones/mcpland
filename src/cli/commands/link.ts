...
export class LinkCommand extends McpLandCommand {
	...
	readEnvVar(destRoot: string, key: string): string | undefined {
	...
	async run(args: string[], _cli: any = {}): Promise<number> {
		...
		const key = this.readEnvVar(root, 'OPENAI_API_KEY');
		if (!key) {
		...
		if (useSSE) {
			outro(
				pc.green(`Updated ${path.relative(root, cursorCfgPath)} for SSE mode`)
			);
			console.log(
				pc.cyan(
					'Make sure to run `mcp serve --port=<port>` to start the SSE server'
				)
			);
		} else {
			outro(
				pc.green(`Updated ${path.relative(root, cursorCfgPath)} for stdio mode`)
			);
		}
